...
*/
	...
	public:
	// constructor
	CarsimCoreAPI(std::string simfile_path);
	// destructor
	~CarsimCoreAPI();
	...
	*/
	int sPrintfError (const char *format, ...);
	// int sDefaultPrintf(const char *format, ...);
	int sGetApi (void *api_func, HMODULE dll, const char *func, const char *dll_name, const char *get_func);
	int sPrintErrorNoDll (const char *where, const char *dll_name);
	int sGetApiRoad(HMODULE dll, const char *dname, const char *me);
