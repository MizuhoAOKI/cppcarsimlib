...
#include <cstring>
#include "json.hpp"
#define IS_NONBLOCKING 0 // 0: blocking, 1: non-blocking
#define MAX_MSG_BYTE_SIZE 1024
...
namespace udp_lib
{
    ...
    　　  */
        Sender(const std::string &destination_address, const int destination_port)
        {
            ...
            const auto val = IS_NONBLOCKING; // 0: blocking, 1: non-blocking
    ...
    public:
         ...
         */
        Receiver(const std::string &source_address, const int receive_port, const int max_msg_byte_size = MAX_MSG_BYTE_SIZE) : max_msg_byte_size_{max_msg_byte_size}
        {
            // Generate socket
            // AF_INET: IPV4, SOCK_DGRAM: UDP, 0: protocol is automoatically chosen
            sock_ = socket(AF_INET, SOCK_DGRAM, 0);
         ...
         * @param max_msg_byte_size : Maximum message buffer size when receiving
         */
        Receiver(const int receive_port, const int max_msg_byte_size = MAX_MSG_BYTE_SIZE) : max_msg_byte_size_{max_msg_byte_size}
        {
            // Generate socket
            // AF_INET: IPV4, SOCK_DGRAM: UDP, 0: protocol is automoatically chosen
            sock_ = socket(AF_INET, SOCK_DGRAM, 0);
