...
int CarsimCoreAPI::sPrintfError(const char *format, ...) {
...
#if (defined(_WIN32) || defined(_WIN64))
HMODULE CarsimCoreAPI::vs_load_library(char* pathDLL) {
  return LoadLibraryA(pathDLL);
}
void CarsimCoreAPI::vs_free_library(HMODULE dll) {
  if (dll != NULL) FreeLibrary(dll);
}
#else
void* CarsimCoreAPI::vs_load_library(char* pathDLL) {
...
int CarsimCoreAPI::vs_get_dll_path(const char* simfile, char* pathDLL)
{
  FILE* fp;
...
int CarsimCoreAPI::sGetApi (void *api_func, HMODULE dll, const char *func, const char *dll_name,
                    const char *get_func) {
#if !defined(_WIN32) && !defined(_WIN64)
  char *errorString;
#endif
...
int CarsimCoreAPI::sPrintErrorNoDll (const char *where, const char *dll_name) {
...
int CarsimCoreAPI::vs_get_api_road (HMODULE dll, const char *dname) {
